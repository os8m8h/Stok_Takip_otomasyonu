...
namespace Stok_Otomasyonu
{
    public partial class frmÜrünEkle : Form
    {
        ...
        private void textBox6_TextChanged(object sender, EventArgs e)
        {
            ...
            baglanti.Open();
                    SqlCommand komut = new SqlCommand("select *from urun where barkodno like '" + BarkodeNotxt.Text + "'", baglanti);
                    SqlDataReader read = komut.ExecuteReader();
                    while (read.Read())
                    {
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("update urun set miktari=miktari+'"+int.Parse(Miktarıtxt.Text)+"'where barkodno='"+BarkodeNotxt.Text+"'",baglanti);
            komut.ExecuteNonQuery();
            baglanti.Close();
            foreach (Control item in groupBox2.Controls)
            {
